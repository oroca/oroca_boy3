...
const char *biosfile  = "/fake86/bios/pcxtbios.bin";
//const char *rom_video = "/fake86/bios/videorom.bin";
//const char *biosfile  = "/fake86/bios/xtbios.bin";
const char *rom_video = "/fake86/bios/et4000.bin";

const char *rom_basic = "/fake86/bios/rombasic.bin";
...
void fake86Main(int argc, const char *argv[])
{
  // setup exit handler
  atexit(exit_handler);
  ...
  if (disk_insert(0, "/fake86/data/dos-boot.img") == true)
  {
    printf("disk_insert OK\n");
  }
  else
  {
    printf("disk_insert Fail\n");
  }
  ...
  if (!emulate_init())
  {
    printf("emulate_init fail\n");
    return;
  }
  // load roms needed by the emulator
  if (!load_roms())
  {
    printf("load_roms fail\n");
    return;
  }
