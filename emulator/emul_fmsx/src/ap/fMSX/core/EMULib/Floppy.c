...
byte *DSKLoad(const char *Name,byte *Dsk,const char *Label)
{
  ...
  if(f_opendir(&D, Name) == FR_OK)
  {
    ...
    while(f_readdir(&D, &fno) == FR_OK)
      if((Path=malloc(strlen(Name)+strlen(fno.fname)+5)))
      {
        ...
        for(J=0;(J<8)&&fno.fname[J]&&(fno.fname[J]!='.');J++)
          FN[J]=toupper(fno.fname[J]);
        for(I=J;I<8;I++) FN[I]=' ';
        for(;fno.fname[J]&&(fno.fname[J]!='.');J++);
        if(fno.fname[J]) J++;
        for(;(I<11)&&fno.fname[J];I++,J++)
          FN[I]=toupper(fno.fname[J]);
        for(;I<11;I++) FN[I]=' ';
        FN[I]='\0';
        ...
        {
          int st_size = f_size(&F);

          /* Allocate input buffer */
          if((Buf=malloc(st_size)))
          {
            UINT len = 0;

            /* Read file into the buffer */
            if((f_read(&F, Buf, st_size, &len) == FR_OK) && len == st_size)
            {
              /* Create and write floppy file */
              if((I=DSKFile(Dsk1,FN)))
                if(DSKWrite(Dsk1,I,Buf,st_size)!=st_size)
                  DSKDelete(Dsk1,I);
            }
...
#define fread(B,L,N,F)  gzread((gzFile)(F),B,(L)*(N))
  ...
  /* Read data */
  if(f_read(&F, Dsk1, DSK_DISK_SIZE, &len) != FR_OK)
  {
    if(!Dsk)
    {
      free(Dsk1);
...
const byte *DSKSave(const char *Name,const byte *Dsk)
{
  ...
  /* If <Name> is a directory... */
  if(f_stat(Name, &FS) == FR_OK && FS.fattrib & AM_DIR)
  {
    /* Compose path name */
...
#define fwrite(B,L,N,F) gzwrite((gzFile)F,(byte *)B,(L)*(N))
  ...
  /* Write data */
  if(f_write(&F,Dsk,DSK_DISK_SIZE,&len) != FR_OK)
  { f_close(&F);return(0); }
